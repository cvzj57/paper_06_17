...
This pseudo-system has a central charge of $Z=1$, and only one electron. It is employed in \textsl{ab-initio} calculations in which several physical characterstics including the HOMO and first ionisation energies, as well as first excitation energy and UV spectra, are found to be well-reproduced by the pseudo-system.
\end{abstract}
...
The aim is to propose a methodology for extracting a potential for a hybridised carbon atom (here an $sp^2$ carbon atom), which can then be used as a building block for constructing chemical systems. This carbon pseudo-atom contains one explicit nuclear charge and one electron.
...
Initially, we optimised potentials for the CH$_3^\bullet$ system.
...
pseudo-potential very difficult to transfert from on system to another.
...
as well as reproduce physically reasonable absorption spectra 
...
\hline
final pseudo-CH$^{\bullet}_{3}$ & -3.910 & 0.624 & 1.5 & 0.5 & -10.000 & - & - \\
\hline
