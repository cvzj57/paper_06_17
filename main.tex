...
Figure \ref{fig:alkenes_hf_dft} show in detail the results obtained with the smaller polyenes.
...
Encouraged by this, we decided to explore the virtual space further by computing a large number of $\pi$ excited states, and to compare the results to all-electron calculations using the PBE0 functional. As can be seen from Figure~\ref{fig:cnhn_uv}, which compares the absorption spectra of pseudo-potential and all-electron calculations for a selection of PAH, the shape of the spectra is well-reproduced. A red-shift of factor 1.15 is applied to the pseudo-potential systems to align them with their all-electron counterparts. We can see from the fit of the pairs of spectra that the scaling generates a near-perfect absorption curve across all systems.
...
\subsubsection*{\sffamily \large Topological analysis of the density}
...
Figure \ref{fig:aim_c6h6} shows the critical points obtained in a reference calculation (details on
how to obtain this analysis are described in the SI)
...
at their position
and are classified according to their sum.
In an all electron AIM analysis, atoms are characterized by three negative eigenvalues
of the hessian of the density noted (3, -3): they are a maximum of the electronic density along all
directions.
Bond critical points are a minimum along the bond direction and a maximum along the normals,
thus noted (3, -1) and cycle critical points are minima on the hyper surface of the cycle
and a maximum along the direction perpendicular to it, so they are noted (3, +1).
A last kind of critical point (cage critical point) is characterized by three positive eigenvalues
(3, +3) but no such point is found in this study.
The critical paths are obtained by connecting the critical points along a direction of null gradient.
Before going further, one should note that, as the critical point analysis were obtained
...
Then, according to this vision, it is not one, but two bond critical points that are to be found,
one above and one below the molecular plane.
Finally, due to the topology of the density, two cycle critical points are found above in below the molecular
plane on the C$_6$ axis of symmetry.
It is remarkable that none of the algorithm we have used to obtain these plots find a cage critical
point exactly at the center of the molecule.
This should be required to fulfill the Poincarré-Hopf relationship.
Yet, after a careful inspection of the results, it turns out that the electronic density
...
The Poincarré-Hopf relationship can be considered to be fulfilled and no more critical points
are to be found.

Figure \ref{fig:aim_c6h6} confirms that the description of the bond pattern is the same
...
However, as both images are obtained with the same scale, 
...
% \hspace*{3in}
% \caption{Comparison of $S^2$ expectation values obtained for the calculation
% of the first triplet configuration in a SCF formalism, for all-electron and pseudo-systems.}
% \label{fig:ssquare}
% \end{figure}
...
\vspace{0.25in}
\hspace*{3in}
\caption{Comparison of the UV spectra obtained with pseudo-potentials (ps) and all-electron (ref) calculations (def2-TZVP/TD-PBE0). The pseudo-potential systems have been red-shifted by a factor of 1.15.}
\label{fig:cnhn_uv}
\end{figure}
...
\vspace{0.25in}
\hspace*{3in}
\caption{\ref{fig:aim_c6h6}Comparison of the bond critical points and bond critical paths obtained at the PBE0/def2-SV(P)
level for the
reference $\pi$ system of benzene (left) and the $\pi$ system obtained with the final
pseudo-potentials (right). Colors indicate the nature of the critical points according to the sign of the Hessian
of the density at their position: in cyan the atom critical points (3, -3) in blue the bond
critical points (3, 1) and in red the cycle critical points (3, -1). The critical paths are drawn in yellow.}
\label{fig:cnhn_uv}
\end{figure}
