...
we intend to use. 
We want to stress here that the gain in computational time is not our goal.
It is clear that replacing bielectronic by a set of monoelectronic integrals
will result in drastic gains on timings.
But our puropose is more to check whether our approach of extracting pseudo-potentials
for hybridised atoms is possible with strictly local pseudo-potentials.
A study of timings is available in the supplementary materials.
...
Let us take the planar CH\(^{\bullet}_{3}\) (d$_{CH}=2.0466 a.u.$) as starting system to be reproduced.
...
However, we can make a more informed guess at some starting parameters from which to optimise.
...
of $0.295$., which leads to a coefficient of $-3.267\;a.u.$.

\subsubsection{Extraction of the six $s$ potentials parameters}
Now that we setup the $p$ potential placed at the nucleus, we shall define
...
Thus, a reparametrization is necessary.
...
Furthermore, we intend to extract parameters on the smallest system possible.
We expect this approach to ensure the most unbiased pseudo-potential parameters. 
...
As represented in Figure \ref{figure:ref_pseudo_diagram}, 6 pseudo-potentials are positionned around
...
\hline
$s$  & $-c$                   & $0$            & $ d$ & $C=1.5$   & $\gamma=0.5$   \\
$s$  & $-c$                   & $0$            & $-d$ & $C=1.5$   & $\gamma=0.5$   \\
$s$  & $\frac{ c\sqrt{3}}{2}$  & $\frac{c}{2}$  & $ d$ & $C=1.5$   & $\gamma=0.5$   \\
$s$  & $\frac{ c\sqrt{3}}{2}$  & $\frac{c}{2}$  & $-d$ & $C=1.5$   & $\gamma=0.5$   \\
$s$  & $\frac{-c\sqrt{3}}{2}$ & $\frac{c}{2}$  & $ d$ & $C=1.5$   & $\gamma=0.5$   \\
$s$  & $\frac{-c\sqrt{3}}{2}$ & $\frac{c}{2}$  & $-d$ & $C=1.5$   & $\gamma=0.5$   \\
...
of the ethylene $\pi$ cloud system with a good accuracy: relative errors on the ionisations
energy and the energies of the HOMO orbital of 80\% and 3\% respectively (Table \ref{tab:res_ourPP}).
...
Calculations are performed at the HF/def2-SV(P) level, values are in atomic eV.}
\end{table}
...
when used in another framework.
...
Furthermore, the parametrization was done with ground state calculations only:
...
Yet, we shall use this potential in the TD-DFT framework, which uses the virtual orbitals.
...
Again, these calculations are succesfull.
...
Taking the optimal parameters we test them against a series of chain alkenes up to length C\(_{12}\)H\(_{14}\), using a variety of functionals. In each case, the geometry of the reference system is optimised according to the method used, 
...
between the lowest triplet (in an unrestricted formalism) and the lowest singlet state
(in a restricted formalism).
Calculations were done at the PBE0/def2-SV(P) level.}
...
TD-DFT First Excitation &   - & 2.6 & 2.8 &  6.3 &  6.9 \\ 
\hline\hline
\end{tabular}
...
TD-DFT First Excitation &    - &  1.0   &   6.1   &   5.6    &   2.9 \\ 
\hline\hline
\end{tabular}
\caption{Mean relative errors (in percent) across methods (HF or different functionals)
...
The model defined here can be used in any wuantum chemistry package, which
implements atomic pseudo-potentials as we provide all the necessary parameters.
We are now working on extending our extraction method to other hybridized atoms
...
\subsection{Timings}
The question of timings was not our initial aim.
Yet, the model that we develop in this work allows for gain in computational time.
To illustrate this, we provide herein a small study done on the C$_{50}$H$_{52}$
alkene chain.
...
basis functions not necessary to the reproduction of the
$\pi$ system.
In other terms, only $p$ functions and higher angular momentum were kept.
...
\hline\hline
\end{tabular}
\caption{\label{tab:time}Time (in second) per SCF iteration.} 
\end{table}
