...
by a series of operators, the which depend on the level of
...
It is clear that replacing the bi-electronic interactions with a set of mono-electronic integrals
will result in drastic gains in timings.
But our puropose is more to see whether our approach of extracting pseudo-potentials
for hybridised atoms is possible with strictly local pseudo-potentials.
A study of timings is available in the supplementary material.
...
It is shown that the properties can be well reproduced save when dealing with
...
Let us take the planar CH\(^{\bullet}_{3}\) (d$_{CH}=2.0466$ $a.u.$, HF/def-SV(P)) as starting system to be reproduced.
...
As represented in Figure \ref{figure:ref_pseudo_diagram}, 6 pseudo-potentials are positioned around
...
the first singlet-triplet $\pi-\pi*$ excitation energy ($\Delta_{ST}$) and
...
Also compared are TD-DFT results for this system and results of some of the authors' previous work, which match to within 3\%.\cite{drujon_pseudopotentials_2013}
...
TD-DFT First Excitation &   - &  3.6  &  2.7 &  6.3 &  7.0 \\ 
\hline\hline
\end{tabular}
\caption{Mean relative errors (in percent) across methods (HF or different functionals)
for ring-systems.
...
\caption{Comparison of the HOMO energy ($\varepsilon_{HOMO}$),
the ionisation energy (I.E.),
the first $\pi-\pi*$ singlet-triplet excitation energy ($\Delta_{ST}$) and
the TD-DFT first excitation energy
between the
