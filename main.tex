...
spectrum of a coronene polymer encapsulated in a carbon nanotube.\cite{nakamura}
...
of a model system (one (19,0) nanotube) the whole system.
We tried to investigate this problem with our pseudo-potentials in the cluster approximation
to see if we could find discrete electronic excitations, which would have a charge transfer
between the coronene and the nanotube character.

An emphasis should be made about the fact that we are trying to use the pseudo-potentials
out of their scope as they were extracted for planar molecules only.
However, if the curvature radii is not too small, we are confident enough that our
...
We make the hypothesis that it is mainly the $\pi$-like electrons of the molecule that are responsible for the
...
diameter ranges from 1.0 to 1.5~nm), which each encapsulate one coronene.
...
some thousands as many $\sigma$ excitations ).
...
ground state to the first 200 excited states (see Supplementary Informations).
...
These calculations, despite their crudeness lead to informative results on the
...
They allow for the use of quantum chemistry method on system for which such calculations
