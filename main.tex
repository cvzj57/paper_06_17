...
of the $s$ potentials as localised as possible, we chose $d=0.5$ a.u., approximately the shortest atom-atom distance permitted in 
QM programs.
...
because of their overlap. In this pargraph we use a simplified definition $\widehat{p}$ of the $p$ pseudo-potential, which allows for a quick evaluation of overlap effects with the basis set: 
\begin{equation}
\chi = e^{-\alpha r^{2}}\sum_{m=-1}^{+1}\left|Y_{1,m}\right>,\qquad \widehat{p} = -Z_{pseudo} | \chi \rangle \langle \chi |
\end{equation}
We define
\begin{equation}
