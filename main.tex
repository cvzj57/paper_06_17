...
This pseudo-system has a central charge of $Z=1$, thus it contains only one
...
one, both of which must be taken into account in order to fulfill chemical requirements.
...
- I could make some comment here about the way in which all these methods have tried to reproduce different molecular features, bond length, mulliken etc. and then make some comparisons with what we do?

- Also question of optimisation algorithm, pseudobonds use training set and powell, sebastiani uses the artificial bee 
colony
]
...
The originality and difficulty of this work resides in the ambition to extract pseudo-potentials
for systems which are not isotropic (hybridized atoms),
and interact in a physically sound manner to reproduce accurately some molecular properties.
This is a major difference with the pseudo-potential based methods, which aim at filling the valency
of an atom (as it is done in the link atom variations).
In this article, we reproduce the $\pi$ system and selected properties 
of various molcules with no $\sigma$ electrons treated explicitely.

This article is structured as follows.
In the first part, the method is defined in detail.
The second part shows the results obtained using the most refined version of the method.
The extracted pseudo-potentials are then employed for extended systems, with results and discussion.
A final perspective is presented in the conclusion.

\section*{\sffamily \Large METHODOLOGY}

In this section, we firstly give the full analytical definition of the pseudo-potential and then
present the steps, which we followed in order to extract the final
pseudo-potential.
This stepwise procedure starts from the smaller system (the bare atom) and reaches the
targeted system (the hybridized sp$^2$ carbon atom in a molecular framework).
Each step benefits from the prvious step, which is used as a starting point.
...
It is possible to obtain the potential parameters entirely by empirical means, in principle.
However, we felt we ought to be able to make a more informed guess at some starting parameters
from which to optimise.
...
In order to make an educated guess of the parameters of this new potential,
we need to find an expression for \(Z_{eff}(\langle r \rangle)\); \(Z_{eff}\) being
the total nuclear attraction the electron would feel in the real CH\(^{\bullet}_{3}\) system,
taking into account the screening effect of the core electrons that would be present
in an all-electron model; and \( \langle r \rangle \) being the expectation distance of the
...
The exponent should be chosen to give us a strong overlap with the molecular
orbital we wish to influence.
Hence we arrive at a \(p_{z}\)-potential that should be physically meaningful.
We chose the exponent in Table \ref{table:p_potentials}, which gave the maximum
overlap possible ($\approx$ 0.79).

\subsubsection*{\sffamily \large CH\(_{3}\): obtaining a guess for the \(s\)-potentials}

We aim now at reproducing the values for the CH\(^{\bullet}_{3}\) radical as given in
Table \ref{table:ch3_s_potentials}. 
This reference CH\(^{\bullet}_{3}\) is created and has its geometry optimised under
Hartree-Fock (HF).
At first we thought to place the $s$-potentials at the C - H bond distance from the
pseudo-carbon ($r_{C - H} = 2.0446$ a.u. in our reference geometry),
before realising such potentials would quickly begin to interfere with other atoms
once we started building molecules with our pseudo-carbon.
Thinking therefore that we would attempt to keep the effects of the $s$-potentials
as closed range as possible, we chose $d=0.5$ a.u.
%, approximately the shortest atom-atom distance
%possible in Turbomole before geometry problems are encountered.

The pseudo-system is then set up, erasing the hydrogen atoms, setting the carbon
charge \(Z_{nucleus} = 1\) and applying \(s\) pseudo-potentials,
...
Figure~\ref{figure:diagram} helps to explain our choice of criteria for gauging the efficacy
of our potentials.
Since our reference system is CH$_3^\bullet$, and we wish ultimately to simulate
$\pi$-bonding, the HOMO energy of CH$_3^{\bullet}$, $\epsilon_{HOMO(CH_3)}$, i.e.
the $p_{z}$ orbital energy, is an obvious choice.

After some trial and improvement we discovered it was much more effective to optimise
the potentials on the ethylene system itself, having constructed a pseudo-ethylene using
the potential setup above. We found it easy to optimise sets of potentials to give us the
HOMO energy of ethylene, which was promising as it suggested that we could maintain an accurate
HOMO energy for the pseudo-system as we added further optimisation constraints.
