...
placed about the pseudo-atom. The $J^{th} \ s$ projector is defined by its $A'_J$ coefficient, its
...
the $A'$ coefficient and $\alpha'$ exponent for the six $s$ potentials.
...
%We start by searching the $p$ potential parameters and, once fixed, we focused on the $s$ potentials. 
...
a guess for the  $p$ potential parameters
 and, once fixed those,
...
%To mimic the effect of an electron-screened nucleus, we use the \(p_{z}\) pseudo-potential.

The  \(p_{z}\) pseudo-potential is thought here to mimic the effect of an electron-screened nucleus.
In order to make an educated guess of the parameters of this new potential, we 
consider a simplification of the pz component of the p potential and we write it as:
\begin{equation}
\chi = e^{-\alpha r^{2}},\qquad \widehat{p} = Z_{pseudo} | \chi \rangle \langle \chi |
\end{equation}

We need to find an expression that can give us a value for \(Z_{eff}\). From an all-electron HF
reference calculation on the CH\(^{\bullet}_{3}\) system, the expected distance of the electron 
from the nucleus \( \langle r \rangle \) can be computed, defined as 
\begin{equation}
\langle r \rangle = \langle \psi_{p_{z}} | r | \psi_{p_{z}} \rangle
\label{equation:exp_r}
\end{equation}

where \(\psi_{p_{z}}\) is the $\pi$ molecular orbital.
...
%\end{equation}
%
%where \(\psi_{p_{z}}\) are the molecular orbitals extracted from a reference calculation of CH\(^{\bullet}_{3}\). 

From Equation \ref{equation:exp_r}, we have \( \langle r \rangle \approx 1.8\;a.u.\) and therefore we can see from Equation \ref{equation:PsirPsi} that \(Z_{eff} \approx 3.6\). 
...
We discuss here the results obtained with the PBE0 functional presented in Figure~\ref{fig:alkenes_hf_dft}.
