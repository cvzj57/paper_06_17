...
give us the effect of the screened nucleus. Since we want a simple expression for
$\chi_{p_z}$, by only one function, the $-{Z_{pseudo}}$ parameter should contain a
...
and we correct by the overlap influence by writing: 
\begin{equation}
Z_{pseudo} = -A = (Z_{eff} - Z_{nucleus})S^{-2}
\end{equation}

%The parameter $\alpha$ will be chosen to maximise the overlap $S$ and the parameter
%$A$ will be computed from $Z_{eff}$ and $S$.
...
For this case study, 
the exponent was chosen to give us a strong overlap with the molecular
orbital we wish to influence.
Hence we arrive at a \(p_{z}\)-potential that should be physically meaningful. 
The optimal values that we found, those which best reproduced the chosen 
physical characteristics of the system, were a coefficient of $A=-2.81$ for 
