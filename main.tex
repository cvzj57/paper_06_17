...
we can say that the $p$ projectors aim to mimic a coulombic interaction (see $\hat{V}(i)$ in Equation~\ref{eq:HuzinagaMPVersion1Potential}),
...
This later role was devoted to the $\hat{\sigma}$ operator in Huzinaga's formulation. \textbf{We also note that the $Z_{c}$ of Equation~\ref{eq:HuzinagaMPVersion1Potential} is also contained within this $\hat{W}$ (see `Results and Discussion' for further detail). This leaves us with the final mono-electronic Hamiltonian operator:}

\begin{equation}
\label{eq:ourmonoelec}
\hat{h}(i) = -\frac{1}{2}\Delta_i - \frac{(Z)}{r_i} + \hat{W}\end{equation}
