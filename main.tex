...
Taking the final potentials, we test them against a series of chain alkenes up to length C\(_{12}\)H\(_{14}\), using a variety of functionals. In each case, the geometry of the all-electron system is optimised according to the method (HF or DFT functional) used, before taking the this geometry and applying the final pseudo-potentials from Table \ref{table:potential_params}. 
...
\caption{DFT (PBE0) comparison of all-electron and pseudo-system energies across a range of chain alkenes.}
\label{fig:alkenes_hf_dft}
\end{figure}
...
\caption{DFT and TD-DFT (PBE0) comparison of all-electron and pseudo-system energies across a range of long chain alkenes (C\(_{20}\)-C\(_{100}\)).}
\label{fig:long_chain_graphs}
\end{figure}
