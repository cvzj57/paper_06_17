...
Finally, this approach is capable of reproducing the $\pi$ electron systems of small or large, planar or non-planar hydrocarbons at low computational cost.
\end{abstract}
...
\hat{h}(i) = -\frac{1}{2}\Delta_i + \sum_{K=1}^N \left[- \frac{(Z_K-Z_{C_K})}{r_{iK}} + \hat{V_K} + \hat{\sigma_K} \right]
\end{equation}
where $\Delta_i$ is the Laplacian of the coordinates of electron $i$, $N$ denotes the number of nuclei and
$Z_{C_K}$ is the number of dormant electrons withdrawn from the $K^{th}$ nucleus of the reference system.
...
The second part shows the results obtained using the most refined version of the method; the extracted pseudo-potentials are then employed for extended planar and non-planar systems, with results and discussion. A final perspective is presented in the conclusion.
...
electronic correlation. The correlation will then be determined by the choice of the method.
...
It is the smallest system containing one and only one sp$^2$-hybridised carbon atom. This choice allows us to isolate the different electrostatic interactions to build a physically meaningful model. We make use of two kinds of Gaussian pseudo-potentials,\cite{me_structure_theory} of \(s\) and \(p\) shapes. 
Figure~\ref{figure:ref_pseudo_diagram} displays the final pseudo-system: The CH\(^{\bullet}_{3}\) radical has been replaced by a hydrogen-like `pseudo-carbon', with a nuclear charge of \(Z_{nucleus} = Z-Z_C = 1\), and one electron occupying the \(p_{z}\) orbital. 
...
As we want to avoid modifying the quantum chemistry software itself, the pseudo-potentials have a semi-local form and we choose to preserve the \(r^{-1}\) behaviour as in Huzinaga's work.
The mono-electronic operator for CH\(^{\bullet}_{3}\) is written as:
\begin{equation}
\label{eq:monoElectronicOperator2}
\hat{h}(i) = -\frac{1}{2}\Delta_i - \frac{1}{r_i} + \hat{W}
\end{equation}
...
This later role was served by the $\hat{\sigma}$ operator in Huzinaga's formulation. 


Our formulation gives multiple variables we can use to manipulate the properties of the system.
...
As the $\sigma$ electrons are not treated explicitly in our model,
...
to other methods and they are employed within the DFT and TD-DFT frameworks for both planar and non-planar systems in the `Applications' section.

\section*{\sffamily \large COMPUTATIONAL DETAILS}

All Hartree-Fock, DFT and time-dependent DFT (TD-DFT) energy calculations are performed with TURBOMOLE 7.1.\cite{TURBOMOLE} The basis set used throughout is def-SV(P).\cite{defsvp} Wherever possible, planar (C\(_{S}\)) symmetry is used. The convergence energy is \(10^{-7}\)~E$_h$ (\texttt{\$scfconv = 7}) for SCF and \(10^{-6}\)~E$_h$ for DFT. All optimisations used the Brent method in SciPy's optimisation library, with a tolerance of \(1.48*10^{-08}\), and used standard Hartree-Fock calculations.\cite{scipy}

Structures are optimised for the ground state at the all-electron level for each functional and for Hartree-Fock,
...
Figures~\ref{fig:alkenes_hf_dft} and \ref{fig:long_chain_graphs} show in detail the results obtained with the PBE0 functional for the small and large polyenes, respectively.
...
the reorganisation of the $\sigma$ electrons in the cation.
For comparison, the mean ionisation errors obtained with our pseudo-potentials are between 7\% (PBE0) and 11\% (TPSS) 
and an average error of 10\% is found in all-electron ROHF calculations for the small-polyenes where all the $\sigma$ doubly-occupied 
orbitals are frozen in the cations, as they are obtained in the calculations for the neutral systems
(see the Supplementary Information, SI, for details). 
...
Since we reproduce the $p$ orbitals well, the aromatic character of the $\pi$ ring
appears.
...
we decided to explore the virtual space further by computing the low-energy UV spectrum (20 first singlet
...
Excitation energies computed with the pseudo-potential are systematically too high, a red scaling factor of
...
The use of our potentials is not restricted to DFT. To demonstrate this, we shall compute at the CASPT2 level the first 
...
the active spaces are (6,6), (10,10), and (10,10), respectively.
...
We were interested in looking at some yet larger and more complex molecules.
In particular, we were curious to see how the potentials would perform in a system that wasn't planar,
as distortions in the plane would, in an all-electron system,
alter the influence of the sp$^2$-hybridised electrons.

%yc
In a comparison between CC2 theoretical calculations and experiment, Nakai \emph{et al.}
show that Electronic Circular Dichroism (ECD) spectra of helicences can be accurately computed.\cite{nakai_theoretical_2012}
First of all let us show here that for small helicenes (6 to 9 rings), it is possible to
reproduce with good accuracy the experimental spectra with DFT (PBE0/def2-SV(P)).

In this work, we are interested in carbon based helicenes made of
ortho-fused-six-member-rings only.
The agreement between experiment, and all-electron CC2 and DFT calculations is very good according to
Figure S-III (see SI).
Relative intensities, sign and positions of the absorption peaks are remarkably
well reproduced by the DFT calculations with no shift on the wavelengths. As a reminder, when the helix followed by a helicene rotates in the counter-clockwise
direction, the helicene is labelled P (plus) and M (minus) otherwise.
The ECD spectra of P helicenes feature a positive absorption at low energy 
followed by a negative absorption about 100 nm further (the spectrum of [8]helicene
is plotted with a thicker line to emphasise this feature). 
This characteristic behavior (in the above defined framework) is indicative
of the chirality M or P of the helicene.
%yc

Figure~\ref{fig:helicene_spectra} displays the UV and ECD spectra for a series of P-[n]helicene molecules of $n=6$ to $n=9$ (structural diagrams are available in the SI),
along with their pseudo-molecular counterparts.
After red-shifting the pseudo-spectra by 32~nm, we can see the general shapes of the pseudo-system UV spectra are qualitatively very similar to the all-electron spectra,
both in their spread and their intensities.
We can also see that many of the peaks of the all-electron spectra appear to be clearly identifiable in the pseudo-spectra. 

The ECD spectra are more complex.
The general shapes of the pseudo-spectra are distorted compared to the all-electron results,
with the distortion increasing from slight to more severe as the helicene becomes longer.
We suggest that the reason for this is that with $n > 6$, the ends of the helicene overlap more and more,
resulting in a much more complex electronic structure.
However, one can still clearly identify the largest peaks for the smaller systems, and the direction of the dichroism remains clear throughout.

These spectra show that we can be confident that our potentials can retain much of the physics of complex $\pi$-systems, even allowing for some distortion of the molecular plane.
These calculations show that the ECD spectra of helicenes are mainly due to the $\pi$-like electrons and
that our pseudo-potentials allow for the reproduction of properties which are much more difficult to reproduce
than UV spectra as they are much more sensitive to the environment.
...
of a model system, which is a (19,0) nanotube.	
...
We chose a non-metallic chiral (1,14) nanotube of an intermediate diameter as the ones used experimentally to	
...
ground state to the first 200 excited states (see SI).	
...
(1) They can be employed with both Hartree-Fock and DFT methods including PBE0, PBE, TPSS and TPSSh functionals. 
CASPT2 calculations have also been positively tested.
(2) They have been shown to function well whether their neighbouring atoms are other pseudo-carbons (as with fused carbon rings), hydrogen atoms (as in CH\(^{\bullet}_{3}\)),
or a mixture of both (as in the polyenes or at the edge of fused ring systems).
(3) They offer a significant gain in computational time as compared to all-electron calculations.
(4) They are fully atomic, meaning that they can be placed relative only to the carbon atom itself and the direction of its bonds. 
(5) We could show that not only were the occupied orbitals well-reproduced by the use of these new potentials, but also that the virtual space is of good quality for excited states calculation. 
...
They are able to lead to the same AIM view of the bonding pattern and to reproduce physically reasonable absorption spectra together with physically-grounded electronic transition densities. Ionisation energies have
also been tested and the expected trends are obtained, even if clearly our
model can not recover the effects of the reorganisation of the $\sigma$ electrons
in the cation. However, deviations
from the all-electron calculations decrease with the size of the system.
(6) They were employed to compute UV and ECD spectra of extended, non-planar systems (provided that the radii are large enough): helicenes
and a coronene embedded in a nanotube. 

The last three points are an improvement over the 2013 formulation of such potentials. Despite the fact that our method does not reach chemical accuracy in ionisation energies, it can be useful to solve highly difficult assignations for large systems in a limited time. 
...
This can be particularly useful in large molecules which are optically active to simulate ECD spectra.
...
Blue indicates a diminution of electronic density and red indicates an increase of electronic density.	
}	
