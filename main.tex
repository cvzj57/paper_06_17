...
bonds. The cyclopentadienyl group was, for instance, successfully extracted. [REF]
In this example, carbon atoms are hybridized and the core/valence distinction is not 
strictly defined. [TRANSITION NEEDED HERE]
...
we make use of two kinds of Gaussian pseudo-potentials \cite{me_structure_theory}, of \(s\) and \(p\) shapes. As we want to avoid modifying the quantum chemistry software itself, the pseudo-potentials have a semi-local form and we choose a value of \(n = 1\) to preserve the \(1/r\) behaviour as in Huzinanaga's work [MORE DETAIL, N IS NOT DEFINED]. As will be discussed in the following sections,
...
%%
 ...
 where \(\psi_{p_{z}}\) are the \(p\) orbitals extracted from a reference calculation of CH\(_{3}\). From Equation \ref{equation:exp_r}, we have \( \langle r \rangle \approx 1.8\;a.u.\) and therefore we can see from Equation \ref{equation:PsirPsi} that \(Z_{eff} \approx 3.6\). Even if we shall use the semi-local pseudopotential definition for the calculations, for a quick evaluation of overlap effects between the basis set and the pseudo-potential, we assume a non-local pseudo-potential definition. We define:
\begin{equation}
...
Calculation Type & HF & PBE0 & PBE & TPSS & TPSSH \\
\hline
Mean \(\pi - \pi*\) Excitation error (\%) & 55.3 & 85.8 & 83.4 & 239.8 & 320.2  \\
...
- Finish long chain calc graphs 

- line 170 reference needed

- 257: n not defined more detail
