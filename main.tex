...
\begin{table}[ht]
\caption{\(s\)-potential fits to ethene \(\pi-\pi*\) excitation (eV).}
\begin{tabular}{c c c c c}
\hline
\(s\)-coefficient & \(s\)-exponent & Triplet - Singlet  & Triplet - Ion  & HOMO  \\
\hline
...
Figure \ref{fig:rings_graphs} shows the triplet-singlet excitation energy, cation-singlet and
HOMO energies for several ring systems.
...
Table \ref{table:long_alkene_errors} gives the same energy details for longer chain alkenes.
The pattern of decreasing cation-singlet and triplet-singlet energies with increasing HOMO
energy is still followed, with the absolute error remaining consistent.
However, with chains of this length we begin to see a divergence in the triplet-singlet
energy between the reference and pseudo-systems. 

Unlike for the previous systems, there is a large discrepancy between delta SCF
and TD-DFT results.
This apparent failure of the pseudo potentials is to be found in the representation
of the triplet state.
If we plot the eigenvalue of the $S^2$ operator for the triplet calculations,
we can see (Figure \ref{fig:ssquare}) that: the spin contamination
of the triplet state computed as a single configuration (\emph{i.e.} in a SCF
framework) increases in both reference and pseudopotential cases.
Yet, this is strenghten in the pseudpotential calculations.
...
Furthermore, in order to show that the recovering of the agreement between the pseudopotential
and the reference calculations is not an artifact, we give in Table~\ref{tab:coef},
the weight and nature of each excitation (weight larger than 3\%)
in the description of the triplet excited state in the prototypical
case of C$_{50}$H$_{52}$ (other values can be found in the SI, which exhibit the same trends).
As can be seen, the qualitative description of the state is in perfect agreement.
...
\begin{table}
\caption{\label{tab:coef}Comparison of the weights (all electron \emph{vs.} pseudopotentials)
...
\caption{Comparison of the eigenvalue of the $S^2$ operator obtained for the calculation
of the first triplet configuration in a SCF formalism, for reference
and pseudo systems.}
\label{fig:ssquare}
\end{figure}

These results show that the potentials that we have extracted are able to reproduce the
